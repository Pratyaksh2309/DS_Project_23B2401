...
# # Load cleaned + ranked data
# df = pd.read_csv("data/summary_ranks.csv")

# # Load BERT embeddings
# bert_embeddings = np.load("models/bert_embeddings.npy")

# # Load BERT model (optional for new queries)
# from sentence_transformers import SentenceTransformer
# bert_model = SentenceTransformer('all-MiniLM-L6-v2')

# # Streamlit app
# st.set_page_config(page_title="Session Summary Finder", layout="wide")
# st.title("🧠 Intelligent Session Summary Finder")
# st.markdown("Enter **topic keywords**, and we'll match you to the most relevant session based on deep semantic similarity using BERT.")

# keywords = st.text_input("🔍 Enter keywords (comma or space separated):")
...
#         # Compute similarity with all summaries
#         similarities = cosine_similarity(query_embedding, bert_embeddings).flatten()

#         # Find the cluster of the best-matching summary
#         best_summary_idx = similarities.argmax()
#         target_cluster = df.iloc[best_summary_idx]['cluster']

#         # Filter summaries from that cluster
#         cluster_df = df[df['cluster'] == target_cluster].sort_values(by='rank_in_cluster').head(3)

#         st.subheader(f"📚 Top 3 Summaries from Cluster {target_cluster}:")
...
if st.button("Find Best Session"):
    if query.strip():
        ...
        top3 = df[df['cluster'] == best_cluster].sort_values('rank_in_cluster').head(3)
...
try:
    ...
    sns.scatterplot(data=umap_df, x='umap_1', y='umap_2', hue='cluster', palette='tab10')
    st.pyplot(fig)
except:
...
selected_cluster = st.selectbox("Choose a cluster to visualize:", sorted(df['cluster'].unique()))
cluster_text = ' '.join(df[df['cluster'] == selected_cluster]['clean_summary'])
...
top_cluster_df = df[df['cluster'] == selected_cluster].sort_values('rank_in_cluster').head(top_k)
